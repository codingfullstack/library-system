<x-layouts::app :title="'Kopija #'.$copy->id">
    @php
        ...
        $copyMeta = [
            ...
            ['icon' => 'bookmark', 'label' => 'Busena', 'badge' => true, 'value' => $statusLabels[$status] ?? $status],
            ['icon' => 'book-open', 'label' => 'Kopija', 'value' => $copy->inventory_code ?: '-'],
            ['icon' => 'tag', 'label' => 'Bruksninis kodas', 'value' => $copy->barcode ?: '-'],
            ['icon' => 'shield-check', 'label' => 'Bukle', 'value' => ucfirst((string) $copy->condition_status) ?: '-'],
            ...
            ['icon' => 'calendar-days', 'label' => 'Isigyta', 'value' => $copy->acquired_at?->format('Y-m-d') ?: '-'],
            ['icon' => 'squares-2x2', 'label' => 'QR reiksme', 'value' => $copy->qr_code ?: '-'],
        ];
    @endphp
    ...
    <x-ui.page class="max-w-none px-4 py-0 sm:px-6 lg:px-8">
        <div class="bg-[#f7f8fa] py-8 dark:bg-zinc-950">
            <div class="mx-auto max-w-[1560px] space-y-6">
                <div class="space-y-3">
                    <div class="flex flex-wrap items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
                        ...
                        @if($copy->book)
                            ...
                            <span>&gt;</span>
                        @endif
                        <span class="text-zinc-900 dark:text-white">Kopija #{{ $copy->id }}</span>
                    </div>

                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <h1 class="text-4xl font-bold tracking-tight text-zinc-950 dark:text-white">Kopija #{{ $copy->id }}</h1>
                            <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                                Perziurekite sios knygos kopijos informacija, bukle, vieta ir veiksmu istorija.
                            </p>
                        </div>

                        <div class="flex flex-wrap items-center gap-3">
                            @can('update', $copy)
                                <a href="{{ route('manage.book-copies.edit', $copy) }}" title="Redaguoti egzemplioriu" class="inline-flex h-11 items-center gap-2 rounded-2xl bg-emerald-700 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-600">
                            ...
                            @can('delete', $copy)
                                <form method="POST" action="{{ route('manage.book-copies.destroy', $copy) }}" onsubmit="return confirm('Ar tikrai nori istrinti si egzemplioriu?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" title="Istrinti egzemplioriu" class="inline-flex h-11 items-center gap-2 rounded-2xl bg-red-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-red-500">
                            ...
                            @if($copy->book)
                                <a href="{{ route('books.show', $copy->book) }}" title="Grizti i knygos puslapi" class="inline-flex h-11 items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 text-sm font-semibold text-zinc-700 shadow-sm transition hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800">
                ...
                @if($errors->any())
                    <x-ui.alert type="error">Nepavyko atnaujinti kopijos gyvenimo ciklo. Patikrink priezasti ir pasirinkta veiksma.</x-ui.alert>
                @endif
                    ...
                    <div class="space-y-6">
                        <section class="overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                            <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                                ...
                                <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Pagrindine informacija apie sios knygos fizine kopija.</p>
                            </div>
                        ...
                        @can('update', $copy)
                            <section class="overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                                <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                                    ...
                                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Valdyk paruosima, sugadinima, tvarkyma, atkurima ir nurasyma su priezastimi.</p>
                                </div>
                                ...
                                <div class="p-5">
                                    @if($copy->activeLoan)
                                        <div class="flex flex-col gap-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/40 dark:bg-amber-950/30">
                                            <div class="flex items-start justify-between gap-4">
                                                <div>
                                                    <div class="text-sm font-semibold text-amber-900 dark:text-amber-200">Kopija siuo metu isduota.</div>
                                                    <p class="mt-1 text-sm text-amber-800 dark:text-amber-300">Gyvenimo ciklo keitimas bus galimas po grazinimo.</p>
                                                </div>
                                    ...
                                    @elseif($copy->availableLifecycleTransitions() === [])
                                        <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950/40 dark:text-zinc-300">
                                            Siam statusui papildomu gyvenimo ciklo veiksmu nebera.
                                        </div>
                                    @else
                                        <form method="POST" action="{{ route('manage.book-copies.lifecycle.update', $copy) }}" class="space-y-4">
                                                ...
                                                @foreach($copy->availableLifecycleTransitions() as $targetStatus)
                                                    <label class="cursor-pointer rounded-2xl border border-zinc-200/80 bg-zinc-50/70 p-4 dark:border-zinc-800 dark:bg-zinc-950/40">
                                                        ...
                                                        <div class="rounded-xl border border-transparent peer-checked:border-teal-500 peer-checked:bg-teal-50 p-1 dark:peer-checked:border-teal-400 dark:peer-checked:bg-teal-950/30">
                                                            ...
                                                            <div class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Busena taps: {{ $statusLabels[$targetStatus] ?? $targetStatus }}</div>
                                                        </div>
                                                    </label>
                                                @endforeach
                                            </div>
                                            ...
                                            <div>
                                                <label for="reason_notes" class="app-label">Priezastis</label>
                                                <textarea id="reason_notes" name="reason_notes" rows="4" class="app-input mt-2" placeholder="Trumpai aprasyk, kas nutiko ir kodel keiciama busena." required>{{ old('reason_notes') }}</textarea>
                                                @error('reason_notes') <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                            </div>

                                            <button type="submit" class="app-button-primary">Issaugoti gyvenimo cikla</button>
                                        </form>
                                    @endif
                                </div>
                            </section>
                        ...
                        <section class="overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                            <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                                <h2 class="text-lg font-semibold text-zinc-950 dark:text-white">Busenos istorija</h2>
                                <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Kas, kada ir del kokios priezasties keite kopijos busena.</p>
                            </div>

                            <div class="p-5">
                                @if($copy->statusHistories->isEmpty())
                                    <x-ui.empty-state title="Istorijos dar nera" description="Kai keisis busena, cia matysi visa gyvavimo cikla." />
                                @else
                                    <div class="space-y-3">
                                                ...
                                                <div class="grid gap-4 px-4 py-4 lg:grid-cols-[220px_220px_minmax(0,1fr)]">
                                                    <div>
                                                        ...
                                                        <div class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                                                            {{ $statusLabels[$history->from_status] ?? ($history->from_status ?: 'Pradine busena') }}
                                                            ->
                        ...
                        @if(auth()->user()?->isSuperAdmin())
                            <section class="overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                                <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                                    ...
                                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Paskutiniai audit irasai, susije su siuo egzemplioriumi.</p>
                                </div>
                                <div class="p-5">
                    ...
                    <div class="space-y-6">
                        <section class="overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                            <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                                ...
                                <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Nuskenuokite mobiliu irenginiu.</p>
                            </div>
                        ...
                        @if($copy->activeLoan)
                            <section class="overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                                <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                                    <h2 class="text-lg font-semibold text-zinc-950 dark:text-white">Aktyvus isdavimas</h2>
                                </div>
                                ...
                                <div class="space-y-4 p-5">
                                    <dl class="space-y-3 text-sm">
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <dt class="text-zinc-500 dark:text-zinc-400">Isdavimo data</dt>
                                            <dd class="text-right font-medium text-zinc-950 dark:text-white">{{ $copy->activeLoan->borrowed_at?->format('Y-m-d H:i') ?: '-' }}</dd>
                                        </div>
                                        <div class="flex items-start justify-between gap-3">
                                            <dt class="text-zinc-500 dark:text-zinc-400">Grazinimo iki</dt>
                        ...
                        @if($copy->activeLoan)
                            <section class="overflow-hidden rounded-[28px] border border-amber-200 bg-amber-50 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/30">
                                <div class="px-5 py-4">
                                    <div class="text-sm font-semibold text-amber-900 dark:text-amber-200">Kopijos busena</div>
                                    <p class="mt-2 text-sm text-amber-800 dark:text-amber-300">
                                        Kopija siuo metu isduota. Gyvenimo ciklo keitimas bus galimas po grazinimo.
                                    </p>
                                </div>
                            </section>
                        @endif
