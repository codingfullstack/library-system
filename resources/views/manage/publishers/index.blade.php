<x-layouts::app :title="'Leidyklos'">
    <x-ui.page class="max-w-none px-4 py-0 sm:px-6 lg:px-8">
        @php
            $visiblePublishers = $publishers->getCollection();
            $withBooks = $visiblePublishers->where('books_count', '>', 0)->count();
            $withoutBooks = $visiblePublishers->where('books_count', '=', 0)->count();
            $countries = $visiblePublishers->pluck('country')->filter()->unique()->count();
        @endphp
                ...
                <section class="overflow-hidden rounded-[24px] border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                    @if($publishers->count())
                                    ...
                                    @foreach($publishers as $publisher)
                                        ...
                                        <tr class="transition hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40">
                                            ...
                                            <td class="px-4 py-4 align-middle">
                                                ...
                                                <div class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ strtoupper(substr($publisher->name, 0, 4)) }}</div>
                                            </td>
                                            <td class="px-4 py-4 align-middle text-sm text-zinc-700 dark:text-zinc-300">{{ $publisher->country ?: '-' }}</td>
                                            <td class="px-4 py-4 align-middle text-center text-sm font-semibold text-emerald-700 dark:text-emerald-300">{{ $publisher->books_count }}</td>
                        ...
                        <div class="flex flex-col gap-4 border-t border-zinc-200 px-5 py-4 text-sm text-zinc-500 dark:border-zinc-800 dark:text-zinc-400 sm:flex-row sm:items-center sm:justify-between">
                            <div>Rodoma {{ $publishers->firstItem() }}-{{ $publishers->lastItem() }} is {{ $publishers->total() }}</div>
