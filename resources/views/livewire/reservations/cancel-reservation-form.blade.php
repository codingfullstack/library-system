<div class="inline-block">
    @if(! $isOpen)
        @if($compact)
            <button type="button" wire:click="open" title="Atsaukti rezervacija" aria-label="Atsaukti rezervacija" class="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-200 bg-white text-zinc-500 transition hover:text-red-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:text-red-300">
    ...
    @else
        <div class="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-4 text-left shadow-sm dark:border-zinc-800 dark:bg-zinc-950/95">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h4 class="text-sm font-semibold text-zinc-950 dark:text-white">Atsaukti rezervacija</h4>
            ...
            <div class="mt-4">
                <label for="cancel-reason-{{ $reservation->id }}" class="app-label">
                    {{ $requiresReason ? 'Atsaukimo priezastis' : 'Priezastis (neprivaloma)' }}
                </label>
                <textarea
                    ...
                    placeholder="Parasykite, kodel rezervacija atsaukiama"
                ></textarea>
                @error('reason')
            ...
            <div class="mt-4 flex justify-end gap-2">
                <button type="button" wire:click="close" class="app-button-secondary">Grizti</button>
                <button type="button" wire:click="save" wire:loading.attr="disabled" class="app-button-danger">
                    <span wire:loading.remove wire:target="save">Patvirtinti atsaukima</span>
                    <span wire:loading wire:target="save">Atsaukiama...</span>
                </button>
            </div>
        </div>
    @endif
