...
<form wire:submit="save" class="space-y-6">
    <div class="grid gap-4 lg:grid-cols-2">
        ...
        <div>
            <label for="email" class="app-label">El. pastas</label>
        ...
        <div>
            <label for="role" class="app-label">Role</label>
        ...
        @if($actor?->isSuperAdmin())
            <div>
                ...
                <select id="libraryId" wire:model.live="libraryId" class="app-input" @disabled($role === 'super_admin')>
                    <option value="">{{ $role === 'super_admin' ? 'Be bibliotekos' : 'Pasirinkti biblioteka' }}</option>
                ...
                <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                    {{ $role === 'super_admin' ? 'Superadmin rolei biblioteka nepriskiriama.' : 'Member rolei nario numeris generuojamas automatiskai.' }}
                </p>
        ...
        <div>
            ...
            <div class="app-input flex items-center bg-zinc-50 text-zinc-600 dark:bg-zinc-950/40 dark:text-zinc-300">
                {{ $previewMembershipNumber ?: ($role === 'member' ? 'Pasirink biblioteka, kad sugeneruoti' : 'Netaikoma') }}
            </div>
        </div>

        <div>
            <label for="password" class="app-label">{{ $isEditing ? 'Naujas slaptazodis' : 'Slaptazodis' }}</label>
            <input id="password" type="password" wire:model="password" class="app-input" {{ $isEditing ? '' : 'required' }}>
            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                {{ $isEditing ? 'Palik tuscia, jei keisti nereikia.' : 'Bent 8 simboliai.' }}
            </p>
            @error('password') <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="passwordConfirmation" class="app-label">Pakartok slaptazodi</label>
    ...
    <div class="flex flex-col gap-3 sm:flex-row">
        <button type="submit" class="app-button-primary" wire:loading.attr="disabled">
            <span wire:loading.remove wire:target="save">{{ $isEditing ? 'Issaugoti pakeitimus' : 'Sukurti vartotoja' }}</span>
            <span wire:loading wire:target="save">{{ $isEditing ? 'Saugoma...' : 'Kuriama...' }}</span>
        </button>
        <a href="{{ route('manage.users.index') }}" class="app-button-secondary">Grizti</a>
    </div>
</form>
