...
class GetRecentAuditLogsForPublisherQuery
{
    /**
     * @return Collection<int, AuditLog>
     */
    public function handle(Publisher $publisher, int $limit = 18): Collection
    {
        $bookIds = Book::query()
        ...
        return AuditLog::query()
            ...
            ->where(function (Builder $query) use ($publisher, $bookIds, $bookCopyIds, $reservationIds, $loanIds) {
                $query
                    ->where(function (Builder $selfQuery) use ($publisher) {
                        $selfQuery->where('auditable_type', $publisher->getMorphClass())
                            ->where('auditable_id', $publisher->id);
                    })
